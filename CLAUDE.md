...
| 3 | `pipeline/classify_papers.py` | **HDBSCAN approximate_predict (원 설계)** — `topic_modeling` 이 저장한 `_hdbscan_model.joblib` 번들(hdbscan_model + UMAP transformer + centroids + tid→cat) 로드 → UMAP 5D 투영 → `hdbscan.approximate_predict` 로 primary sub-cluster 결정. Outlier(-1)는 768D centroid 코사인 최단점으로 강제 배정. `all_categories` 는 centroid 거리 오름차순 top-N parent. LLM 호출 없음. **`.venv312` (Python 3.12) 필수** — UMAP 의존 |
...
**`.venv312` (Python 3.12) 는 `topic_modeling.py` + `classify_papers.py` 전용이다.** 나머지 모든 스크립트는 시스템 Python(`python`, 3.14)으로 실행한다.

이유: Windows Smart App Control(WDAC)이 Python 3.14의 numba/llvmlite DLL을 차단하여 `umap-learn`이 시스템 Python에서 동작하지 않는다. 두 스크립트가 UMAP에 의존한다:
- `topic_modeling.py`: `UMAP.fit_transform` (학습)
- `classify_papers.py`: `umap_cluster.transform` (학습된 transformer로 신규 임베딩 5D 투영) + `hdbscan.approximate_predict` (원 설계의 density-faithful 분류)

`run_update_force.py` 의 `TOPIC_MODELING_PYTHON` 변수가 두 스크립트 호출을 자동으로 `.venv312` 로 라우팅한다.

```bash
# venv 생성 (최초 1회)
python -m uv venv .venv312 --python 3.12
python -m uv pip install --python .venv312/Scripts/python.exe \
  umap-learn hdbscan joblib numpy anthropic scikit-learn sentence-transformers

# topic_modeling / classify_papers 는 .venv312 로 실행
...
# 분류만 단독 — `.venv312` 필수 (UMAP transform + hdbscan.approximate_predict)
# 사전 조건: topic_modeling 이 `_hdbscan_model.joblib` 번들을 미리 저장해 두었어야 함
PYTHONUTF8=1 .venv312/Scripts/python.exe pipeline/classify_papers.py --topic ai4s
PYTHONUTF8=1 .venv312/Scripts/python.exe pipeline/classify_papers.py --topic ai4s --slugs 088,1093 --dry-run

# Topic modeling 도 .venv312 사용 (UMAP/hdbscan/sentence-transformers 의존)
PYTHONUTF8=1 .venv312/Scripts/python.exe pipeline/topic_modeling.py --topic ai4s
